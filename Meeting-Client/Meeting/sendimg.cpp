...
void SendImg::pushToQueue(QImage img)
{
    ...
    img.save(&buf, "JPEG");
    QByteArray ss = qCompress(byte);//压缩函数
    QByteArray vv = ss.toBase64();//转换为字节码
//    qDebug() << "加入队列:" << QThread::currentThreadId();
    queue_lock.lock();
    while(imgqueue.size() > QUEUE_MAXSIZE)//如果队列中传输数据大于1500则阻塞线程，等待消费者线程消费数据
    {
        queue_waitCond.wait(&queue_lock);
    }
    imgqueue.push_back(vv);
    queue_lock.unlock();
    queue_waitCond.wakeOne();//唤醒消费者线程
}
...
{
    QMutexLocker locker(&m_lock);
    //QMutexLocker就是相当于c++中的unique_lock函数，在其生命周期中加锁并且解锁
    m_isCanRun = false;
}
