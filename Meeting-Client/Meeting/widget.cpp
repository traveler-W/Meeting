...
extern LogQueue *logqueue;//使用extern 函数修饰告诉编译器我在这里只声明日志写入类，并不在此定义
    ...
    , ui(new Ui::Widget)
{
    ...
    qDebug() << "main: " <<QThread::currentThread();//打印主线程的线程ID
    qRegisterMetaType<MSG_TYPE>();//后续在信号与槽中实用自定义类型需要注册此类型此，但是一般在跨线程中会报错，需要使用新式的信号与槽写法

    WRITE_LOG("-------------------------Application Start---------------------------");
    WRITE_LOG("main UI thread id: 0x%p", QThread::currentThreadId());//也是对于日志文件的写入操作，不过不太清晰具体用法
    //ui界面
    _createmeet = false;
    _openCamera = false;
    _joinmeet = false;
    ...
    //----------------------------------------------------------
    //文本传输(局部线程)
    //内置队列进行把消息进行压缩或者处理压入内置队列，本是一个线程类，run()中是取出消息然后加入发送队列
...
void Widget::datasolve(MESG *msg)
{
    if(msg->msg_type == CREATE_MEETING_RESPONSE)
    {
        ...
        if(roomno != 0)
        {
            ...
            ui->mainshow_label->setPixmap(QPixmap::fromImage(QImage(":/myImage/sese.jpg").scaled(ui->mainshow_label->size())));
        }
        else
        {
            _createmeet = false;
    ...
    else if(msg->msg_type == TEXT_RECV)
    {
        ...
        //qDebug() <<"来自"<<QHostAddress(msg->ip).toString()<<"的消息:"<<str;
